...
- The use of the input elements datatable (see Section \@ref(inputelements)) to select elements while avoiding hardcoding.\
...
- A "wipe data" internal feature that is applied whenever `Limit results to query only` is `No`. It removes all pre-existing data points corresponding to the years for which the plugin is run.  
